...
import com.example.lostandfoundappcs230.databinding.ActivityFoundPostEditBinding
...
class FoundPostEdit : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.image4FoundEdit.setOnClickListener {
            selectImage4()
        }
        binding.image5FoundEdit.setOnClickListener {
            selectImage5()
        }
...
//            val fileName = formatter.format(now)
            ...
            db.collection("Found Items").document(fileName).set(userMap)
                .addOnSuccessListener {
                    ...
                    binding.image4FoundEdit.setImageResource(R.drawable.resource_new)
                    binding.image5FoundEdit.setImageResource(R.drawable.resource_new)
                }
...
//        shows a progress dialog
        ...
        if (Image4Uri != null) {
            storage4.putFile(Image4Uri!!)
                .addOnSuccessListener {
                    Toast.makeText(this@FoundPostEdit, "4th Image uploaded", Toast.LENGTH_SHORT)
                        .show()
                    if (progressDialog.isShowing) progressDialog.dismiss()
        ...
        if (Image5Uri != null) {
            storage5.putFile(Image5Uri!!)
                .addOnSuccessListener {
                    Toast.makeText(this@FoundPostEdit, "5th Image uploaded", Toast.LENGTH_SHORT)
                        .show()
                    if (progressDialog.isShowing) progressDialog.dismiss()
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 400 && resultCode == RESULT_OK) {
            Image4Uri = data?.data!!
            binding.image4FoundEdit.setImageURI(Image4Uri)
        }
        if (requestCode == 500 && resultCode == RESULT_OK) {
            Image5Uri = data?.data!!
            binding.image5FoundEdit.setImageURI(Image5Uri)
        }
    }
    ...
    private fun readData(userID: String) {
        ...
        if (image4url != "null"){
            Glide.with(this)
                .load(image4url).into(binding.image4FoundEdit)
        }else{
            binding.image4FoundEdit.setImageResource(R.drawable.resource_new)
        }
        if (image5url != "null"){
            Glide.with(this)
                .load(image5url).into(binding.image5FoundEdit)
        }else{
            binding.image5FoundEdit.setImageResource(R.drawable.resource_new)
        }
    }
